...
class BinahApp {
  ...
  updateConnectionStatus() {
    ...
    if (!window.binahAuth || !window.binahAuth.isLoggedIn()) {
      // Ocultar status bar para usuários não logados
      const statusBar = document.getElementById('statusBar');
      if (statusBar) statusBar.style.display = 'none';
      return;
    }
    
    // Mostrar status bar para usuários logados
    const statusBar = document.getElementById('statusBar');
    ...
    // Só mostrar indicadores de database se for admin
    const indicator = document.getElementById('dbIndicator');
    const statusText = document.getElementById('dbStatusText');
    
    if (!isAdmin) {
      // Ocultar apenas os indicadores técnicos para usuários comuns
      if (indicator) indicator.style.display = 'none';
      if (statusText) statusText.style.display = 'none';
      return;
    }
    
    // Para admins - mostrar indicadores técnicos
  ...
  selectLiveResult(topicId) {
    ...
    // NÃO ativar modo de busca aqui para evitar resultados zerados
  }
...
${window.binahAuth.canEditPost(topic.author) ? `
  ...
  async viewTopic(id) {
    try {
      ...
      // Incrementar view count a cada clique
  ...
  closeTopicPopup(event) {
    ...
    if (popup) {
      ...
      // Restore body scroll and reset position
...
${window.binahAuth.canEditPost(topic.author) ? `
  ...
  async markHelpful(topicId, helpful) {
    if (helpful) {
      // Verificar se usuário já deu like neste tópico
      const likedTopics = JSON.parse(localStorage.getItem('binah_liked_topics') || '[]');
      
      if (likedTopics.includes(topicId)) {
        this.showNotification('Você já avaliou este tópico como útil!', 'warning');
        return;
      }
      ...
      try {
        ...
        // Salvar que usuário deu like neste tópico
        likedTopics.push(topicId);
        localStorage.setItem('binah_liked_topics', JSON.stringify(likedTopics));
  ...
  async saveTopic(event) {
    ...
    // Usuários logados podem criar tópicos
...
function resetLocalStorage() {
  ...
  localStorage.removeItem('binah_liked_topics');
...
function showSavedData() {
  ...
  const liked = localStorage.getItem('binah_liked_topics');
  
  if (data) {
    const topics = JSON.parse(data);
    console.log('📊 Dados salvos:', topics.length, 'tópicos');
    console.table(topics.map(t => ({ id: t.id, title: t.title, category: t.category, views: t.views, helpful: t.helpful })));
  } else {
    console.log('❌ Nenhum tópico salvo no localStorage');
  }
  
  if (liked) {
    console.log('👍 Tópicos curtidos:', JSON.parse(liked));
  } else {
    console.log('👍 Nenhum like registrado ainda');
  }
}
...
async function loadProfileData() {
  if (!window.binahAuth || !window.binahAuth.isLoggedIn()) return;
  
  const user = window.binahAuth.getCurrentUser();
  
  // Update user info
  document.getElementById('profileUsername').textContent = user.username;
  document.getElementById('profileRole').textContent = user.role === 'admin' ? 'Administrador' : 'Usuário';
  document.getElementById('profileEditUsername').value = user.username;
  document.getElementById('profileEditRole').value = user.role === 'admin' ? 'Administrador' : 'Usuário';
  document.getElementById('profileLastLogin').value = new Date().toLocaleString('pt-BR');
  document.getElementById('profileJoinDate').textContent = '2024';
  
  // Calculate statistics
  await calculateUserStats();
  
  // Load user topics by default
  loadUserTopics();
}
...
async function calculateUserStats() {
  try {
    ...
    const userTopics = allTopics.filter(topic => topic.author === user.username);
    ...
    userTopics.forEach(topic => {
      totalViews += topic.views || 0;
      totalLikes += topic.helpful || 0;
    });
    
    // Update display
    document.getElementById('profilePostsCount').textContent = userTopics.length;
    document.getElementById('profileTotalViews').textContent = totalViews.toLocaleString('pt-BR');
    document.getElementById('profileTotalLikes').textContent = totalLikes.toLocaleString('pt-BR');
...
async function loadUserTopics() {
  try {
    ...
    const container = document.getElementById('userTopicsList');
    ...
    if (userTopics.length === 0) {
      container.innerHTML = `
        <div class="loading-topics">
          ...
          <button onclick="openNewTopicModal(); closeUserProfile();" class="btn-create-post" style="margin-top: 15px;">
    ...
    container.innerHTML = userTopics.map(topic => `
      <div class="topic-item" onclick="viewPostFromProfile(${topic.id})">
        <div class="topic-title">${topic.title}</div>
        <div class="topic-meta">
          <div>
            <span style="background-color: ${app.getCategoryColor(topic.category)}20; color: ${app.getCategoryColor(topic.category)}; padding: 4px 8px; border-radius: 4px; font-size: 11px;">
              ${app.getCategoryIcon(topic.category)} ${topic.category}
            </span>
            <span style="margin-left: 10px; color: var(--secondary);">
              📅 ${app.formatDate(topic.date)}
            </span>
          </div>
...
function saveNewPassword(event) {
  ...
  app.showNotification('Senha alterada com sucesso!', 'success');
  closeChangePassword();
}
